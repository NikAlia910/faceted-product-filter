...
public class ComprehensiveApiPerformanceTest extends Simulation {
    ...
    ChainBuilder createProduct() {
        return exec(session -> {
            ...
            return session
                ...
                .set("productImageUrl", "http://example.com/image.jpg")
                .set("productRating", "4.5");
        })
            .exec(
                http("Create Product")
                    .post("/api/products")
                    ...
                    .check(header("Location").exists().saveAs("productLocation"))
            )
            .exitHereIfFailed()
            .exec(session -> {
    ...
    ChainBuilder partialUpdateProduct() {
        return exec(session -> {
            String productId = session.getString("productId");
            String patchDesc = "Patched_" + productId + "_" + (System.currentTimeMillis() % 10000);
            return session.set("patchDescription", patchDesc);
        })
            .exec(
