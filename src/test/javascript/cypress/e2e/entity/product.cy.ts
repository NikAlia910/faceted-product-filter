...
describe('Product e2e test', () => {
  ...
  const productSample = { name: 'woot accelerator', price: 28270.21 };
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      method: 'POST',
      url: '/api/categories',
      body: { name: 'responsible scornful', description: 'wriggler' },
    }).then(({ body }) => {
      category = body;
    });
  });
  ...
  describe('new Product page', () => {
    ...
    it('should create an instance of Product', () => {
      cy.get(`[data-cy="name"]`).type('save aha breed');
      cy.get(`[data-cy="name"]`).should('have.value', 'save aha breed');

      cy.get(`[data-cy="description"]`).type('../fake-data/blob/hipster.txt');
      cy.get(`[data-cy="description"]`).invoke('val').should('match', new RegExp('../fake-data/blob/hipster.txt'));

      cy.get(`[data-cy="price"]`).type('12816.34');
      cy.get(`[data-cy="price"]`).should('have.value', '12816.34');

      cy.get(`[data-cy="imageUrl"]`).type('indeed');
      cy.get(`[data-cy="imageUrl"]`).should('have.value', 'indeed');

      cy.get(`[data-cy="rating"]`).type('0.62');
      cy.get(`[data-cy="rating"]`).should('have.value', '0.62');
